...
class SupabaseServiceClass {
  ...
  async initialize() {
    ...
    if (!supabaseUrl || !supabaseKey) {
      throw new Error(
        "Supabase URL et clé manquantes dans les variables d'environnement",
      );
    }
  ...
  private notifyArticleChange(payload: any) {
    window.dispatchEvent(
      new CustomEvent("supabase-article-change", { detail: payload }),
    );
  }

  private notifyCategoryChange(payload: any) {
    window.dispatchEvent(
      new CustomEvent("supabase-category-change", { detail: payload }),
    );
  }
  ...
  async signIn(email: string, password: string): Promise<ExtendedUser | null> {
    ...
    if (userError) {
      ...
      const profileData = {
        ...
        niveau_acces:
          data.user.user_metadata?.niveau_acces || "lecture_modification",
      };
  ...
  async getArticles(): Promise<Article[]> {
    ...
    return data.map((item) => ({
      ...
      pieces_jointes: item.pieces_jointes
        ? JSON.parse(item.pieces_jointes)
        : [],
    }));
  }
  ...
  async addArticle(article: Omit<Article, "id">): Promise<void> {
    ...
    const articleData: SupabaseArticle = {
      ...
      pieces_jointes: article.pieces_jointes
        ? JSON.stringify(article.pieces_jointes)
        : null,
    };
  ...
  async getSousCategories(categorie?: string): Promise<SousCategorie[]> {
    ...
    let query = this.supabase.from("sous_categories").select("*").order("nom");
  ...
  private async addToHistory(
    ...
    const { error } = await this.supabase.from("modification_history").insert({
      table_name: table,
      record_id: recordId,
      field_name: "general",
      old_value: null,
