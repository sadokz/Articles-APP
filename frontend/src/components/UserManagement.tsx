  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description:
          "Impossible de charger les données. Vérifiez votre connexion et vos permissions.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
              ...
              {(!hasPermission("users.read") || !hasPermission("roles.read")) &&
                (hasPermission("users.read") ||
                  ...
                  <div className="mt-2 p-2 bg-yellow-100 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-sm">
                    ⚠️ Permissions limitées : Vous n'avez accès qu'à certaines
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
            <TabsList
              className={`grid w-full ${availableTabs.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
            >
              {hasPermission("users.read") && (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setUserForm({ ...userForm, role_id: parseInt(value) })
                }
              >
                <SelectTrigger>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) =>
                  setRoleForm({ ...roleForm, is_active: checked })
                }
              />
                        ...
                        {resourcePermissions.map((permission) => (
                          ...
                          >
                            <Checkbox
                              ...
                              checked={
                                (roleForm.permission_ids as number[])?.includes(
                                  permission.id,
                                ) || false
                              }
                              onCheckedChange={(checked) =>
